...
public class InitialDataLoader implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (userRepository.existsByEmail("admin@bn-system.com")) {
            ...
            if (postRepository.count() == 0) {
                System.out.println("Post 데이터를 생성합니다...");
                createPostData();
                System.out.println("Post 데이터 생성 완료!");
            } else {
                System.out.println("Post 데이터도 이미 존재합니다.");
            }
        ...
        Project testProject = Project.builder()
                ...
                .description("Swagger 테스트를 위한 샘플 프로젝트입니다")
        ...
        // 4. 프로젝트 멤버 추가
        ProjectMember projectMember = ProjectMember.create(testProject, admin, ProjectRole.ADMIN);
    ...
    private void createPostData() {
        ...
        Project testProject = projectRepository.findAll().stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("No project found"));

        // Step 조회 (orderIndex 기준)
        Step step1 = stepRepository.findAll().stream()
                .filter(s -> s.getOrderIndex() == 1)
                .findFirst()
                .orElse(null);

        Step step2 = stepRepository.findAll().stream()
                .filter(s -> s.getOrderIndex() == 2)
                .findFirst()
                .orElse(null);

        Step step3 = stepRepository.findAll().stream()
